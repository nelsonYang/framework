 ...
 */
public class FlashUploadParameterReceiveWorkHandler implements WorkHandler {

    private final Logger logger = LogFactory.getInstance().getLogger(FlashUploadParameterReceiveWorkHandler.class);
    private WorkHandler nextHandler;
    private String[] allowFiles = {".rar", ".doc", ".docx", ".zip", ".pdf", ".txt", ".swf", ".wmv", ".gif", ".png", ".jpg", ".jpeg", ".bmp", ".mp3", ".mp4"};

    public FlashUploadParameterReceiveWorkHandler(WorkHandler nextHandler) {
    ...
    public void execute() {
        ...
        if (contentType == null || contentType.toLowerCase().contains("application/x-www-form-urlencoded")) {
            ...
            try {
                ...
                for (FileItem item : list) {
                    ...
                    if (!"request".equals(fieldName) && !"flashSessionId".equals(fieldName)) {
                    ...
                    } else if ("flashSessionId".equals(fieldName)) {
                        in = item.getInputStream();
                        byte[] buf = new byte[2048];
                        logger.debug("item.size=".concat(String.valueOf(item.getSize())));
                        in.read(buf);
                        value = new String(buf, "UTF-8");
                        logger.debug("falshSessionId =" + value);
                        if (value != null && !value.isEmpty()) {
                            httpSession.setAttribute("flashSessionId", value.trim());
                        } else {
                            isValidateFileName = false;
                            break;
                        }
        ...
        if (isSuccess) {
            ...
            if (!isValidateFileName) {
                applicationContext.getResponseWriter(ResponseTypeEnum.NO_DATA_JSON).toWriteErrorMsg(8000, "参数验证错误或者falshid为空");
            }
        }
